...
if [[ -z "$latest_tcpdump" || -z "$latest_libpcap" || -z "$latest_strace" || -z "$latest_gdb" || -z "$latest_nmap" ]]; then
...
git add upstream/tcpdump upstream/libpcap upstream/strace upstream/gdb upstream/nmap

changed=false
if ! git diff --cached --quiet -- upstream/tcpdump upstream/libpcap upstream/strace upstream/gdb upstream/nmap; then
  changed=true
fi
