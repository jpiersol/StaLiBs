...
.PHONY: build build-tcpdump build-strace build-gdb build-nmap package verify clean update-upstream

build: build-tcpdump build-strace build-gdb build-nmap
...
verify:
	./scripts/verify-static.sh dist/bin/tcpdump-linux-$(ARCH) dist/bin/strace-linux-$(ARCH) dist/bin/gdb-linux-$(ARCH) dist/bin/nmap-linux-$(ARCH)
