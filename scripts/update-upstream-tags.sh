...
latest_lmdb="$(latest_tag "$lmdb_repo" 'LMDB_*' '^LMDB_[0-9]+\.[0-9]+\.[0-9]+$')"
...
if [[ -z "$latest_tcpdump" || -z "$latest_libpcap" || -z "$latest_strace" || -z "$latest_gdb" || -z "$latest_nmap" || -z "$latest_jq" || -z "$latest_curl" || -z "$latest_openssl" || -z "$latest_socat" || -z "$latest_bind9" || -z "$latest_lmdb" || -z "$latest_mtr" || -z "$latest_lsof" || -z "$latest_iproute2" || -z "$latest_wireshark" || -z "$latest_ripgrep" ]]; then
...
current_lmdb="$(git -C upstream/lmdb describe --tags --exact-match 2>/dev/null || true)"
...
echo "Current LMDB:     ${current_lmdb:-unknown}"
echo "Latest LMDB:      $latest_lmdb"
...
git -C upstream/lmdb checkout -q "$latest_lmdb"
...
git add upstream/tcpdump upstream/libpcap upstream/strace upstream/gdb upstream/nmap upstream/jq upstream/curl upstream/openssl upstream/socat upstream/bind9 upstream/lmdb upstream/mtr upstream/lsof upstream/iproute2 upstream/wireshark upstream/ripgrep

changed=false
if ! git diff --cached --quiet -- upstream/tcpdump upstream/libpcap upstream/strace upstream/gdb upstream/nmap upstream/jq upstream/curl upstream/openssl upstream/socat upstream/bind9 upstream/lmdb upstream/mtr upstream/lsof upstream/iproute2 upstream/wireshark upstream/ripgrep; then
  changed=true
fi
