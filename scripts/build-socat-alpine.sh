...
cp -a "$repo_root/upstream/socat" "$src_dir/socat"
rm -rf "$src_dir/socat/.git"

socat_src="$src_dir/socat"
...
binary="$socat_src/socat"
out="$dist_bin/socat-linux-$arch"
cp "$binary" "$out"
strip "$out" || true
chmod 0755 "$out"
...
buildinfo="$dist_meta/socat-linux-$arch.buildinfo.txt"
{
  echo "tool=socat"
  echo "artifact=socat-linux-$arch"
  echo "arch=$arch"
  ...
  echo "socat_tag=$git_tag"
  echo "socat_commit=$(git -C "$repo_root/upstream/socat" rev-parse HEAD 2>/dev/null || true)"
